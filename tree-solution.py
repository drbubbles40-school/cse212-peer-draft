...
class BST:
    ...
    #################Problem 1##################
    #Write code to take any tree and balance it. 
    #Feel free to experiment to find a more efficient solution
    #NOTE: This BST class does not support duplicate number values
    #NOTE: This is much simpler using existing methods
    def balance(self):
        '''
        Balances the sorted tree
        '''
        temp_array = [] #Create an empty list to fill in ascending order
        for data in self:
            temp_array.append(data)
        
        self.root = None #Reset the tree
        self._insert_middle(temp_array, 0 , len(temp_array) - 1) #use the insert_middle function to refill the tree
...
print(f'Unbalanced Height: {tree.get_height()}') #15

tree.balance()
print(f'Balanced Height: {tree.get_height()}')  #4